      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          {popularRoutes.map((route, index) => (
            <motion.div
              key={route.id}
              className="group glass rounded-3xl p-6 border border-white/20 dark:border-gray-700/50 hover:bg-white/30 dark:hover:bg-black/40 transition-all duration-500 cursor-pointer relative overflow-hidden"
              whileHover={{ 
                scale: 1.02, 
                y: -4,
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center space-x-2">
                  ...
                  {route.ecoFriendly && (
                    <span className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 px-2 py-1 rounded-lg text-xs font-medium flex items-center">
                      🌱 Eco
                    </span>
                  )}
                  {route.premium && (
                    <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-400 px-2 py-1 rounded-lg text-xs font-medium flex items-center">
                ...
                {route.surgePricing && (
                  <motion.div 
                    className="flex items-center bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-400 px-3 py-1 rounded-full text-xs font-medium"
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredRoutes.map((route, index) => (
            <motion.div
              key={route.id}
              className="glass rounded-2xl p-5 border border-white/20 dark:border-gray-700/50 hover:bg-white/25 dark:hover:bg-black/35 transition-all duration-300 cursor-pointer hover:shadow-lg hover:shadow-red-500/10"
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 px-4 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
          >
            ← Back
          </button>
        </div>
          ...
          <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-3">
              ...
              <select
                ...
                className="p-3 glass border border-white/20 dark:border-gray-700/50 rounded-xl text-gray-900 dark:text-white text-lg min-w-[120px]"
              >
                {[1, 2, 3, 4, 5].map(num => (
            ...
            {passengers > 1 && (
              <motion.span 
                className="text-lg text-green-600 dark:text-green-400 font-medium flex items-center"
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-mono">
                ${selectedRoute.surgePricing 
                  ? (selectedRoute.fare * selectedRoute.surgeMultiplier * passengers).toFixed(2)
                  : (selectedRoute.fare * passengers).toFixed(2)
                }
              </span>
            </div>
            ...
            {passengers > 1 && (
              <div className="flex justify-between items-center text-green-600 dark:text-green-400">
                ...
                <span className="font-mono">
                  -${(selectedRoute.fare * passengers * groupDiscounts[passengers] * 
                    (selectedRoute.surgePricing ? selectedRoute.surgeMultiplier : 1)).toFixed(2)}
                </span>
              </div>
            )}
